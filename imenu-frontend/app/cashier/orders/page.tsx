...
export default function OrdersPage() {
  ...
  async function fetchOrders() {
    try {
      ...
      const errorMsg = buildError(err, "Failed to load orders");
  ...
  async function fetchOrderItems(orderId: number) {
    ...
    try {
      ...
      const errorMsg = buildError(err, "Failed to load order items");
      setErrorMessage(errorMsg);
      setMessage(null);
      showToast(errorMsg, "error");
    }
  ...
  async function handleConfirmedAction() {
    ...
    try {
      ...
      fetchOrders();
      // Refresh order items for the expanded order
      if (expandedOrderId) {
  ...
  const handleConfirmAction = async (type: "delete" | "update", payload: any) => {
    const actionText = type === "delete" ? "delete this item" : "update this item";
    ...
    >
      ...
      <div className="mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className={`text-sm font-medium mt-1 ${theme === "dark" ? "text-gray-400" : "text-gray-600"}`}>
        ...
        <button
          ...
          className="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg font-semibold shadow transition-all duration-300 hover:scale-105"
        >
          + Create Order
        </button>
      </div>
      ...
      <div className="mb-4">
        <input
          ...
          className={`w-full md:w-96 px-4 py-2 rounded-lg border transition-colors duration-300 focus:outline-none focus:ring-2 focus:ring-emerald-500 ${
            theme === "dark"
      ...
      {errorMessage && (
        <div
          className={`mb-4 p-3 rounded-lg border whitespace-pre-line ${
            theme === "dark"
                    ...
                    >
                      ...
                      <td className="p-4 text-center">
                        <button
                          ...
                          className="px-3 py-1 bg-emerald-600 hover:bg-emerald-700 text-white rounded text-xs shadow transition-all duration-200"
                        >
                    ...
                    {expandedOrderId === order.id && (
                      <tr>
                        <td colSpan={6} className="p-6">
                          <div
                            className={`rounded-xl shadow p-4 transition-colors duration-300 ${
                          ...
                          >
                            ...
                            <div
                              className={`grid grid-cols-[2fr_1fr_1fr_1fr_1fr_1fr_auto] text-xs font-bold mb-2 ${
                            ...
                            {orderItems.map((item) => (
                              <div
                                key={item.id}
                                className="grid grid-cols-[2fr_1fr_1fr_1fr_1fr_1fr_auto] gap-2 mb-2"
                              >
                                <input
